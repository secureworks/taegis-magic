...
def process_correlate_netflow(
    ...
    *,
    ...
    for chunk in chunk_list(pids, 100):
        netflow_correlation_ids = [
            NetflowCorrelationId(part[0], part[1], part[2])
            for part in (pid.split(":") for pid in chunk)
        ]

        query = template.render(
            table=NETFLOW, filters=netflow_correlation_ids, earliest=f"-{earliest}"
        )
    ...
    netflow_df[f"{merge_on}"] = (
        netflow_df["host_id"] + ":" + netflow_df["processcorrelationid.pid"]
    )
    netflow_df.loc[~has_colon, f"{merge_on}"] = (
        netflow_df["host_id"]
        + ":"
        + netflow_df["processcorrelationid.pid"]
        + ":"
        + netflow_df["processcorrelationid.timewindow"]
    )
